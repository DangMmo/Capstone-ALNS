# File: DataStructures.py (DA SUA LOI LOGIC TAI TRONG)

import sys
from Parser import ProblemInstance 

class SERoute:
    # ... (phần __init__ và __repr__ giữ nguyên, chúng đã hiển thị đúng logic tải trọng) ...
    ...
    def __repr__(self):
        ...
        s.append(f"--- SERoute for Satellite {self.satellite.id} (Cost: {self.total_dist:.2f}) --- {route_deadline_str}")
        ...
        final_load = current_load - self.total_load_pickup
    ...
    def calculate_full_schedule_and_slacks(self):
        ...
        for i in range(n - 2, -1, -1):
            ...
            travel_dist = self.problem.get_distance(node_obj.id, succ_node_obj_lookup.id) if succ_node_obj_lookup else 0
            slack_between = (self.service_start_times.get(succ_node_id, float('inf')) - (self.service_start_times.get(node_id, 0) + node_obj.service_time) - travel_dist)
    ...
    # ### BẮT ĐẦU KHỐI LOGIC ĐƯỢC THAY ĐỔI HOÀN TOÀN ###
    def find_best_insertion_pos(self, customer):
        ...
        for i in range(1, len(self.nodes_id)):
            # --- 1. MÔ PHỎNG LẠI TẢI TRỌNG VÀ KIỂM TRA TÍNH KHẢ THI ---
            # Xây dựng tuyến đường giả định nếu chèn khách hàng vào vị trí i
            temp_nodes_id = self.nodes_id[:i] + [customer.id] + self.nodes_id[i:]

            # Tính tổng tải giao/nhận mới cho tuyến giả định
            temp_total_delivery = self.total_load_delivery
            temp_total_pickup = self.total_load_pickup
            if customer.type == 'DeliveryCustomer':
                temp_total_delivery += customer.demand
            else: # PickupCustomer
                temp_total_pickup += customer.demand
                
            # Nếu tổng tải ban đầu đã vượt quá capacity, bỏ qua
            if temp_total_delivery > capacity:
                continue

            # Mô phỏng tải trọng tức thời qua từng điểm
            is_capacity_feasible = True
            current_load = temp_total_delivery # Tải trọng khi bắt đầu từ vệ tinh
            
            # Duyệt qua các khách hàng trong tuyến giả định
            for node_idx in range(1, len(temp_nodes_id) - 1):
                ...
                if cust_obj.type == 'DeliveryCustomer':
                    current_load -= cust_obj.demand
                else: # PickupCustomer
                    current_load += cust_obj.demand
                
                # Kiểm tra tải trọng tức thời sau mỗi lần phục vụ
                if current_load > capacity:
                    is_capacity_feasible = False
                    break # Vi phạm, không cần kiểm tra nữa
            
            if not is_capacity_feasible:
                continue # Chuyển sang vị trí chèn tiếp theo

            # --- 2. KIỂM TRA TÍNH KHẢ THI VỀ THỜI GIAN (LOGIC CŨ VẪN ĐÚNG) ---
            ...
            time_shift = arrival_at_succ - self.service_start_times.get(self.nodes_id[i], 0)
            if time_shift > self.forward_time_slacks.get(self.nodes_id[i], float('inf')): 
                continue

            # --- 3. TÍNH TOÁN CHI PHÍ (NẾU MỌI THỨ ĐỀU KHẢ THI) ---
    ...
    def insert_customer_at_pos(self, customer, pos):
        ...
        cost_increase = (self.problem.get_distance(prev_node_obj.id, customer.id) + self.problem.get_distance(customer.id, succ_node_obj.id) - self.problem.get_distance(prev_node_obj.id, succ_node_obj.id))
...
class FERoute:
    ...
    def __repr__(self):
        ...
        s.append(f"--- FERoute (Cost: {self.total_dist:.2f}, Total Pickup: {total_pickup:.2f}, Route Deadline: {route_deadline:.2f}) ---")
        s.append(f"Path: {path_str}")
    ...
    # ### THAY ĐỔI: Chấp nhận các tham số tùy chọn cho add_satellite ###
    def add_satellite(self, satellite, delivery_load, pickup_load, deadline=float('inf')):
        """
        Thêm một vệ tinh vào tuyến. Nếu vệ tinh đã tồn tại, cập nhật tải trọng.
        """
        if satellite.id not in self.nodes_id:
            self.nodes_id.insert(-1, satellite.id)
        
        # Cộng dồn tải trọng
        if satellite.id not in self.satellite_loads:
            self.satellite_loads[satellite.id] = {'delivery_load': 0, 'pickup_load': 0}
        ...
        if satellite.id not in self.satellite_deadlines:
            self.satellite_deadlines[satellite.id] = deadline
        else:
            self.satellite_deadlines[satellite.id] = min(self.satellite_deadlines[satellite.id], deadline)
    ...
    def update_route_info(self):
        ...
        for i in range(len(self.nodes_id) - 1):
            ...
            if curr_node_id != self.problem.depot.id:
                travel_time = self.problem.get_distance(prev_node_id, curr_node_id)
        ...
        if len(self.nodes_id) > 2:
             self.schedule['final_arrival'] = current_time + self.problem.get_distance(self.nodes_id[-2], self.nodes_id[-1])
        else:
