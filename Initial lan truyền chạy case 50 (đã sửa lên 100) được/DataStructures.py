...
class SERoute:
    ...
    def insert_customer_at_pos(self, customer, pos):
        # BƯỚC 1: Xác định các nút lân cận (tiền nhiệm & kế nhiệm) TRƯỚC KHI thay đổi tuyến đường.
        # Nút tiền nhiệm là nút ở vị trí `pos - 1`.
        # Nút kế nhiệm là nút ở vị trí `pos`.
        prev_node_obj = self.problem.node_objects[self.nodes_id[pos-1] % self.problem.total_nodes]
        succ_node_obj = self.problem.node_objects[self.nodes_id[pos] % self.problem.total_nodes]
        
        # BƯỚC 2: Tính toán sự thay đổi về chi phí dựa trên các nút lân cận đã xác định.
        ...
        # BƯỚC 3: Bây giờ mới thực hiện việc chèn và cập nhật các thuộc tính.
        self.nodes_id.insert(pos, customer.id)
        self.total_dist += cost_increase
        if customer.type == 'DeliveryCustomer': 
            self.total_load_delivery += customer.demand
        else: 
            self.total_load_pickup += customer.demand

        # BƯỚC 4: Tính toán lại toàn bộ lịch trình cho tuyến đường mới.
        self.calculate_full_schedule_and_slacks()
