...
# ==============================================================================
# HÀM TRỢ GIÚP NỘI BỘ CHO VIỆC CHÈN (ĐÃ TÁI CẤU TRÚC)
# ==============================================================================
def _perform_insertion(solution: "Solution", context: "ChangeContext", customer_to_insert: "Customer", best_option: Dict):
    """
    Thực hiện việc chèn tại chỗ, có sao lưu và theo dõi qua context.
    """
    problem = solution.problem
    option_type = best_option.get('type')

    # Backup các route sẽ bị ảnh hưởng TRƯỚC khi chèn
    ...
    if option_type == 'insert_into_existing_se':
        ...
        else: # Trường hợp đặc biệt (hiếm): SE route không còn được phục vụ
        ...
        context.track_new_route(new_se) # Theo dõi route mới
        new_fe = FERoute(problem)
        solution.add_fe_route(new_fe)
        context.track_new_route(new_fe) # Theo dõi route mới
    ...
    else:  # Không tìm thấy lựa chọn chèn khả thi
        if customer_to_insert not in solution.unserved_customers:
            solution.unserved_customers.append(customer_to_insert)
...
# ==============================================================================
# CÁC TOÁN TỬ SỬA CHỮA (REPAIR OPERATORS) (ĐÃ TÁI CẤU TRÚC)
# ==============================================================================
def greedy_repair(solution: "Solution", context: "ChangeContext", customers_to_insert: List["Customer"]):
    ...
    # Dùng list() để tạo bản sao, tránh sửa đổi list gốc khi đang duyệt
...
def regret_insertion(solution: "Solution", context: "ChangeContext", customers_to_insert: List["Customer"], k: int = 4):
    ...
    while remaining_customers:
        ...
        for customer in remaining_customers:
            ...
            best_cost = best_options[0]['total_cost_increase']
            regret = sum(opt['total_cost_increase'] - best_cost for opt in best_options[1:])
...
# Các toán tử sắp xếp theo tiêu chí cũng được sửa đổi tương tự
