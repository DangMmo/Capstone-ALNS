# --- START OF FILE Heuristics.py (FULL INTEGRATED INSERTION HEURISTIC) ---
...
def create_integrated_initial_solution(problem: 'ProblemInstance', random_customers: bool = True) -> 'VRP2E_State':
    ...
    for i, customer in enumerate(customers_to_serve):
        ...
        for se_route in solution.se_routes:
            ...
            if not se_route.serving_fe_routes: continue # Can co FE route de tinh toan
            ...
            # Tim dung SE route tham chieu trong ban sao cua FE route
            temp_se_route_ref = next(r for r in temp_fe_route.serviced_se_routes if r.satellite.id == se_route.satellite.id)
        ...
        for satellite in problem.satellites:
            ...
            temp_fe_for_new.add_serviced_se_route(temp_new_se) # Lien ket tam thoi
