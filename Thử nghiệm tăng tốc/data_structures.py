# --- START OF FILE data_structures.py (FIXED) ---
...
class SERoute:
    # ... (phần __init__ đến get_customers() giữ nguyên) ...
    ...
    ### --- START: HÀM MỚI CHO THỬ NGHIỆM TỐI ƯU HÓA --- ###
    def calculate_insertion_properties(self, customer: "Customer", pos: int) -> Optional[Dict]:
        """
        Tính toán các thuộc tính và tính khả thi của việc chèn một khách hàng
        vào vị trí `pos` MÀ KHÔNG THỰC SỰ SỬA ĐỔI ROUTE.

        Trả về một dictionary chứa các thông tin cần thiết nếu khả thi,
        nếu không trả về None.
        """
        problem = self.problem

        # --- 1. Mô phỏng việc chèn và kiểm tra tải trọng ban đầu ---
        temp_nodes_id = self.nodes_id[:pos] + [customer.id] + self.nodes_id[pos:]
        ...
        for node_id in temp_nodes_id[1:-1]:
            # <<< SỬA LỖI Ở ĐÂY: Thêm % problem.total_nodes >>>
            cust_obj = problem.node_objects[node_id % problem.total_nodes]
        ...
        # --- 2. Mô phỏng việc tính toán lại lịch trình và chi phí ---
        temp_service_start_times = {}
        start_time = self.service_start_times.get(self.nodes_id[0], 0.0)
        temp_service_start_times[temp_nodes_id[0]] = start_time

        for i in range(len(temp_nodes_id) - 1):
            prev_id, curr_id = temp_nodes_id[i], temp_nodes_id[i+1]
            # <<< SỬA LỖI Ở ĐÂY: Thêm % problem.total_nodes >>>
            prev_obj = problem.node_objects[prev_id % problem.total_nodes]
            curr_obj = problem.node_objects[curr_id % problem.total_nodes]
            
            st_prev = prev_obj.service_time if prev_obj.type != 'Satellite' else 0.0
            departure_prev = temp_service_start_times.get(prev_id, 0.0) + st_prev
            arrival_curr = departure_prev + problem.get_travel_time(prev_obj.id, curr_obj.id)
            
            start_service = max(arrival_curr, getattr(curr_obj, 'ready_time', 0))
            temp_service_start_times[curr_id] = start_service

            if start_service > getattr(curr_obj, 'due_time', float('inf')) + 1e-6:
                return None

        # --- 3. Tính toán chi phí và đóng gói kết quả ---
        new_total_dist = 0.0
        for i in range(len(temp_nodes_id) - 1):
            prev_id, curr_id = temp_nodes_id[i], temp_nodes_id[i+1]
            # <<< SỬA LỖI Ở ĐÂY: get_distance đã dùng ID thật, không cần modulo >>>
            # Lấy đối tượng thật để lấy ID thật
            prev_obj = problem.node_objects[prev_id % problem.total_nodes]
            curr_obj = problem.node_objects[curr_id % problem.total_nodes]
            new_total_dist += problem.get_distance(prev_obj.id, curr_obj.id)

        deadlines = {
            # <<< SỬA LỖI Ở ĐÂY: Thêm % problem.total_nodes >>>
            c.deadline for nid in temp_nodes_id 
            if (c := problem.node_objects[nid % problem.total_nodes]).type == 'PickupCustomer' and hasattr(c, 'deadline')
        }
        
        schedule_start_time = temp_service_start_times[temp_nodes_id[0]]
        schedule_end_time = temp_service_start_times[temp_nodes_id[-1]]

        return {
            "cost_increase": new_total_dist - self.total_dist,
            "new_total_dist": new_total_dist,
            "new_delivery_load": new_delivery_load,
            "new_pickup_load": new_pickup_load,
            "new_schedule_start_time": schedule_start_time,
            "new_schedule_end_time": schedule_end_time,
