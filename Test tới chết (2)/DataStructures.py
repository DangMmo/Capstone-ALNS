# --- START OF FILE DataStructures.py (FINAL CORRECTED VERSION) ---
...
class SERoute:
    ...
    def find_best_insertion_pos(self, customer):
        ...
        for i in range(1, len(self.nodes_id)):
            ...
            is_cap_ok = True
            temp_del_load = self.total_load_delivery
            if customer.type == 'DeliveryCustomer':
                temp_del_load += customer.demand
            if temp_del_load > self.problem.se_vehicle_capacity:
                continue

            temp_nodes_for_cap_check = self.nodes_id[:i] + [customer.id] + self.nodes_id[i:]
            ...
            cost_increase = temp_route.total_dist - self.total_dist
    ...
    def is_feasible_under_proxy_deadline(self):
        ...
        # Service time tại coll_id của vệ tinh trong tuyến SE là 0
...
class Solution:
    ...
    def calculate_total_cost_and_time(self):
        ...
        if not self.fe_routes:
            ...
            # Tổng thời gian là thời điểm hoạt động cuối cùng của xe FE
            self.total_time = max(r.schedule[-1]['arrival_time'] for r in self.fe_routes if r.schedule)

# --- END OF FILE DataStructures.py (FINAL CORRECTED VERSION) ---