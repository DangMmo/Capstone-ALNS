...
class SERoute:
    ...
    def is_feasible_under_proxy_deadline(self):
        ...
        if not pickup_customers:
            return True

        effective_route_deadline = min(c.deadline for c in pickup_customers)
        
        collection_satellite_node_id = self.nodes_id[-1]
        arrival_at_collection_satellite = self.service_start_times.get(collection_satellite_node_id, float('inf'))
        
        collection_satellite_obj = self.satellite
        depot_obj = self.problem.depot

        proxy_arrival_at_hub = (arrival_at_collection_satellite + 
                                collection_satellite_obj.service_time + 
                                self.problem.get_travel_time(collection_satellite_obj.id, depot_obj.id))
    ...
    def find_best_insertion_pos(self, customer):
        ...
        for i in range(1, len(self.nodes_id)):
            ...
            if temp_total_delivery > self.problem.se_vehicle_capacity: continue
            ...
            for node_idx in range(1, len(temp_nodes_id) - 1):
                ...
                if current_load > self.problem.se_vehicle_capacity: is_capacity_feasible = False; break
            if not is_capacity_feasible: continue
            
            # B2 & 3: Time Window & Slack Check
...
class FERoute:
    ...
    def update_route_info(self):
        ...
        final_arrival_time = self.schedule.get('final_arrival', 0.0)
        departure_time = self.schedule.get(self.problem.depot.id, {}).get('departure', 0.0)
        self.total_time = final_arrival_time - departure_time
