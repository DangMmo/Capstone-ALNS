...
class ProblemInstance:
    def __init__(self, file_path):
        ...
        for index, row in df.iterrows():
            node = None
            if row['Type'] == 0:
                node = Depot(index, row['X'], row['Y'])
                self.depot = node
            elif row['Type'] == 1:
                node = Satellite(index, row['X'], row['Y'], row['Service Time'])
                self.satellites.append(node)
            elif row['Type'] == 2:
                node = DeliveryCustomer(index, row['X'], row['Y'], row['Demand'], row['Service Time'], row['Early'], row['Latest'])
                self.customers.append(node)
            elif row['Type'] == 3:
                node = PickupCustomer(index, row['X'], row['Y'], row['Demand'], row['Service Time'], row['Early'], row['Latest'], row['Deadline'])
                self.customers.append(node)
            if node:
        ...
        # <<< THÊM MỚI: Định nghĩa tốc độ xe >>>
        # Dựa trên bài báo của Zamal et al., tốc độ trung bình là 350 mét/phút
        self.vehicle_speed = 350.0 # mét/phút (giả định giống nhau cho cả FE và SE)
        
        # Tạo ma trận khoảng cách (đơn vị: mét)
    ...
    def get_distance(self, node_id1, node_id2):
        """Trả về khoảng cách Euclidean (mét) giữa hai điểm."""
        return self.dist_matrix.get(node_id1, {}).get(node_id2, float('inf'))

    # <<< THÊM MỚI: Hàm tính thời gian di chuyển >>>
    def get_travel_time(self, node_id1, node_id2):
        """
        Tính thời gian di chuyển (phút) giữa hai điểm dựa trên khoảng cách và tốc độ.
        """
        distance = self.get_distance(node_id1, node_id2)
        if self.vehicle_speed > 0:
            return distance / self.vehicle_speed
        return float('inf') # Tránh lỗi chia cho 0
